...
```
Текущее состояние класса не просматривается 'снаружи', но есть возможность (**которой нужно обязательно пользоваться**) анализировать возвращаемые ошибки. Практически все методы класса умеют их возвращать.

**Граф далёк от простоты и представлен здесь только для того, чтобы автор библиотеки не забыл как работает его собственный код. Пользоваться gia_tga можно и без знания графа.**
