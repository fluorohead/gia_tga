...
**Граф конечных состояний далёк от простоты и он скорее нужен автору, чтобы помнить как работает его собственная библиотека. Поэтому предлагаю не обращать внимания, чтобы не запутаться. **
...
```
 QImage img(decoded_data, info.width, info.height, info.bytes_per_line, Image::Format_RGBA8888);
 QLabel label;
