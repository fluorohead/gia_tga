...
InvalidHeader--validate_header-->HeaderValidated;
...
```
Текущее состояние хранится внутри класса и 'снаружи' состояния не видно, но есть возможность (**которой нужно обязательно пользоваться - it's mandatory!**) анализировать возвращаемые ошибки. Практически все методы класса умеют их возвращать.

**Т.к. граф конечных состояний далёк от простоты, то предлагаю не обращать на него внимания и пользоваться следующими готовыми вариантами вызовов.**

В следующей последовательности отражены все методы класса **GIA_TgaDecoder** и последовательность их вызовов :
...
|--|--|:--:|
|**init**|Через него в класс передаётся указатель на исходный TGA-ресурс и размер в байтах. Под передачей не подразумевается никакой move-семантики. Класс не начинает владеть ресурсом и не берёт на себя ответственности по его освобождению. Никакого копирования ресурса внутрь класса GIA_TgaDecoder тоже не происходит. Класс просто работает с указателем. Память исходного ресурса можно изменять или высвободить только после вызова метода decode. Если вы сделаете это где-то в промежутке, то с большой вероятностью получите UB, поэтому ни в коем случае не делайте этого после передачи указателя в GIA_TgaDecoder. Метод init можно вызывать многократно, таким образом "переключая" один и тот же экземпляр класса GIA_TgaDecoder на работу со следующим TGA-файлом. Одновременно класс работает только с одним ресурсом.|Не возвращает.|
|**validate_header**|Проверяет TGA-заголовок на корректность. В качестве параметров указывается максимальное разрешение (по-умолчанию это 8192x16384). При выходе за пределы пиксельных размеров или, если обнаружатся некорректные сочетания полей заголовка, класс переходит в состояние InvalidHeader, возвращается ошибка с таким же названием InvalidHeader. Выйти из этого состояния можно только через повторные вызовы init + validate_header (само собой уже на другом исходном ресурсе). В случае удачи класс переходит в состояние HeaderValidated (возвращает 'ошибку' ValidHeader) и становится возможным вызов остальных методов. Если предварительно не был вызван init, то вернётся NotInitialized.|*ValidHeader*, *InvalidHeader*, *NotInitialized*|
|**info**|Необязательный метод. Возвращает структуру типа GIA_TgaInfo с информацией из TGA-заголовка и футера (при его наличии). Данные будут корректны только в случае, если предшествующий вызов validate_header вернул ValidHeader.|Не возвращает.|
|**decode**|Декодирует исходные данные в байт-массив с форматом пикселей **QImage::Format_RGBA8888**. Один пиксель занимает 4 байта (32-бит), где 3 байта отводятся под RGB и один под Alpha. Последовательность хранения цветовых составляющих BB GG RR AA, т.е. самый первый байт отвечает за Blue, следующий за Green и т.д. При удачном декодировании возвращается Success и класс переходит в состояние DecodedOK. Но в процессе декодирования могут произойти и неудачи. Например метод не сможет получить необходимый объём памяти : возвратит MemAllocErr и перейдёт в состояние NotEnoughMem. Исходные данные окажутся обрезанными (truncated) : возвратит TruncDataAbort и перейдёт в DecodingAbort. В исходных RLE-пакетах обнаружатся неожиданные дополнительные пиксели : возвратит TooMuchPixAbort и переход в DecodingAbort.|*TruncDataAbort*, *TooMuchPixAbort*, *Success*, *MemAllocErr*, *NeedHeaderValidation*|
